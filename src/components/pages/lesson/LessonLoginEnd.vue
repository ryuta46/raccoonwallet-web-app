<template>
  <div style="min-height: 100vh">
    <tool-bar back="/lesson/login" :title="$t('lesson.login_end_title')"></tool-bar>
    <v-content app>
      ...
      <v-layout raw wrap style="margin-bottom: 40px">
        <v-flex xs10 offset-xs1>
          <p class="text-xs-center">
            <span v-for="sentence in $t('lesson.login_end_message')">{{ sentence }}<br></span>
          </p>
        </v-flex>
      </v-layout>

      <v-layout class="app-bottom-container" column align-center>
        <v-btn color="primary" small to="/top">OK</v-btn>
      </v-layout>
    </v-content>
  </div>
