<template>
  <div>
    <tool-bar back="/wallet/creation/new" :title="$t('wallet.end_title')"></tool-bar>

    <v-content app>
      <img src="../../../assets/heroimage_new_large.png" style="width: 100%">
      <v-layout raw wrap>
        <v-flex xs10 offset-xs1>
          <p class="text-xs-center">{{ $t('wallet.end_message_0') }}<br>{{ $t('wallet.end_message_1') }}</p>
        </v-flex>

        <v-flex xs10 offset-xs1>
          <p class="text-xs-center">{{ $t('wallet.end_message_2') }}<br>{{ $t('wallet.end_message_3') }}</p>
        </v-flex>
